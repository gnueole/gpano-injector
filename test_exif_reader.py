...
class TestExifReader(unittest.TestCase):
    ...
    def test_xmp_display(self):
        """Test that -f option displays XMP metadata if present."""
        # 1. Create a file with XMP
        output_file = "test_exif-360.jpg"
        if os.path.exists(output_file):
            os.remove(output_file)
        ...
        # 2. Read it back
        result = subprocess.run(
            [sys.executable, "exif_reader.py", "-f", output_file],
            capture_output=True,
            text=True,
        )

        # 3. Verify XMP tags are displayed
        self.assertIn("XMP Metadata:", result.stdout)
        self.assertIn("GPano:ProjectionType", result.stdout)
...
class TestExifReader(unittest.TestCase):
    ...
    def test_help_contains_version(self):
        ...
        result = subprocess.run(
            [sys.executable, "exif_reader.py", "-h"], capture_output=True, text=True
        )
